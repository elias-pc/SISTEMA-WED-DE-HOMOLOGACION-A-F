...
const distributionSchema = z.object({
    ...
    if (value.mode === 'balanced' && !value.executiveIds?.length)
        context.addIssue({ code: 'custom', path: ['executiveIds'], message: 'Selecciona al menos una ejecutiva.' });
    if (value.mode === 'quantity' && !value.quantities?.length)
        context.addIssue({ code: 'custom', path: ['quantities'], message: 'Define las cantidades por ejecutiva.' });
});
async function processScope(client, processId, user) {
    const result = await client.query(`SELECT id,company_id FROM homologation_processes WHERE id=$1`, [processId]);
    if (!result.rowCount)
        return { error: 'Proceso no encontrado.', status: 404 };
    if (!canAccessCompany(user, String(result.rows[0].company_id)))
        return { error: 'Proceso fuera de tu alcance.', status: 403 };
    return { companyId: String(result.rows[0].company_id) };
}
assignmentsRouter.get('/portfolio', async (request, response) => {
    ...
    try {
        ...
        if ('error' in scope)
            return response.status(scope.status).json({ error: scope.error });
...
assignmentsRouter.post('/assign', validateBody(assignmentSchema), async (request, response) => {
    ...
    const result = await applyPlan(request.user, body.processId, plan, body.reason);
...
assignmentsRouter.post('/unassign', validateBody(unassignmentSchema), async (request, response) => {
    ...
    try {
        ...
        if (providers.size !== providerIds.length) {
            await client.query('ROLLBACK');
...
async function applyPlan(user, processId, plan, reason) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        ...
        if ('error' in scope) {
            ...
            Object.assign(error, { statusCode: scope.status });
            throw error;
        }
        const providerIds = [...new Set(plan.map((item) => item.providerId))];
        if (providerIds.length !== plan.length) {
            const error = new Error('La selección contiene proveedores duplicados.');
      ...
      WHERE u.id=ANY($1::text[]) AND uc.company_id=$2 AND u.role='ejecutiva' AND u.active=true`, [executiveIds, scope.companyId]);
        if (validExecutives.rowCount !== executiveIds.length) {
            const error = new Error('Una o más ejecutivas no son válidas para esta empresa.');
            Object.assign(error, { statusCode: 422 });
            throw error;
        }
        const providers = await lockProviders(client, processId, providerIds);
        if (providers.size !== providerIds.length) {
            const error = new Error('Uno o más proveedores no pertenecen al proceso.');
            Object.assign(error, { statusCode: 422 });
            throw error;
        }
...
async function lockProviders(client, processId, providerIds) {
