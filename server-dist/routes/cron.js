...
import { legacyProviderStatus, validateTransition } from '../workflow.js';
import { queueWorkflowNotification } from '../notifications.js';
export const cronRouter = Router();
cronRouter.get('/certificates', async (request, response) => {
    if (!config.cronSecret)
        return response.status(503).json({ error: 'CRON_SECRET no está configurado.' });
    if (request.get('authorization') !== `Bearer ${config.cronSecret}`)
        return response.status(401).json({ error: 'Cron no autorizado.' });
    ...
    try {
        ...
        for (const row of candidates.rows) {
            const from = { step: Number(row.current_step), status: row.workflow_status, substatus: row.workflow_substatus };
            const transition = expiryTransition(from.substatus, String(row.valid_until), new Date());
            if (!transition)
                continue;
            const validation = validateTransition(from, transition, 'sistema', {});
            if (!validation.ok)
                continue;
            ...
            if (transition === 'MARCAR_VENCIDO')
                ...
                counters.porVencer += 1;
        }
        await client.query('COMMIT');
        response.json({ ok: true, ...counters });
    }
    catch (error) {
