...
export function notificationTemplateForTransition(transition) {
    return templateByTransition[transition] || null;
}
export async function queueWorkflowNotification(client, input) {
    ...
    const id = randomUUID();
    await client.query(`INSERT INTO provider_notifications(id,provider_id,channel,template_code,payload,status,failure_reason,created_by_user_id)
     VALUES($1,$2,'WHATSAPP',$3,$4::jsonb,$5,$6,$7)`, [id, input.providerId, template, JSON.stringify(input.payload), status, omission, input.actorUserId || null]);
