...
function scoped(user, companyId) { return user.role === 'supervisor_general' || user.empresaIds.includes(companyId); }
...
memoryRouter.post('/companies', requireUser, (request, response) => { const user = response.locals.user; if (user.role !== 'supervisor_general')
...
memoryRouter.post('/processes', requireUser, (request, response) => { const user = response.locals.user; if (user.role !== 'supervisor_general')
...
memoryRouter.get('/providers', requireUser, (request, response) => { const user = response.locals.user, process = processes.find(item => item.id === request.query.processId); if (!process || (!scoped(user, process.empresaId) && process.ejecutivaId !== user.id))
    return response.status(403).json({ error: 'Proceso fuera de alcance.' }); response.json({ providers: providers.filter(item => item.procesoId === process.id) }); });
memoryRouter.post('/providers', requireUser, (request, response) => { const user = response.locals.user; if (user.role === 'cliente')
    return response.status(403).json({ error: 'No tienes permiso.' }); const provider = { ...request.body, id: request.body.id || randomUUID(), estado: 'En proceso', estadoEjecutiva: 'Contactado', calificacion: 0, fechaRegistro: new Date().toISOString().slice(0, 10), vigencia: 'N/A' }; providers.unshift(provider); response.status(201).json({ provider }); });
memoryRouter.patch('/providers/:id/status', requireUser, (request, response) => { const user = response.locals.user, provider = providers.find(item => item.id === request.params.id); if (!provider)
    return response.status(404).json({ error: 'Proveedor no encontrado.' }); if (!canUpdateStatus(user.role, request.body.estado))
    return response.status(403).json({ error: 'Estado no permitido para tu rol.' }); if (user.role === 'ejecutiva')
    provider.estadoEjecutiva = request.body.estado;
else
    provider.estadoSupervisor = request.body.estado; response.json({ provider }); });
memoryRouter.post('/users', requireUser, (request, response) => { const user = response.locals.user; if (user.role !== 'supervisor_general')
