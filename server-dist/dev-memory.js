...
memoryRouter.get('/providers', requireUser, (request, response) => { const user = response.locals.user, process = processes.find(item => item.id === request.query.processId); if (!process || (!scoped(user, process.empresaId) && process.ejecutivaId !== user.id))
    return response.status(403).json({ error: 'Proceso fuera de alcance.' }); response.json({ providers: providers.filter(item => item.procesoId === process.id).map(item => ({ ...item, transicionesDisponibles: memoryTransitions(item, user) })) }); });
...
memoryRouter.get('/providers/:id/workflow', requireUser, (request, response) => { const user = response.locals.user, provider = providers.find(item => item.id === request.params.id); if (!provider)
    return response.status(404).json({ error: 'Proveedor no encontrado.' }); response.json({ provider, transicionesDisponibles: memoryTransitions(provider, user), historial: workflowHistory.get(provider.id) || [] }); });
