...
export async function persistTransitionRecords(client, input) {
    ...
    if (transition === 'ASIGNAR_EJECUTIVA') {
        ...
        if (!existing.rowCount)
            await client.query(`INSERT INTO provider_assignments(id,provider_id,assignment_role,assigned_user_id,assigned_by_user_id,reason) VALUES($1,$2,'ejecutiva',$3,$4,$5)`, [randomUUID(), providerId, executiveId, actorUserId, text(data.motivo) || null]);
    }
...
export async function providerDossier(client, providerId) {
