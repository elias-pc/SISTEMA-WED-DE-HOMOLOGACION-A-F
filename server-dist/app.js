...
app.use((request, response, next) => {
    const requestOrigin = `${request.protocol}://${request.get('host')}`;
    if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(request.method) && request.headers.origin && request.headers.origin !== requestOrigin && !config.allowedOrigins.includes(request.headers.origin))
        return response.status(403).json({ error: 'Origen no permitido.' });
    next();
});
app.get('/api/health', async (_request, response) => {
    if (config.useMemoryDatabase)
        return response.json({ status: 'ok', storage: 'memory' });
    try {
        await pool.query('SELECT 1');
        response.json({ status: 'ok', storage: 'postgresql' });
    }
    catch (error) {
        console.error('[health] PostgreSQL no disponible', error);
        response.status(503).json({ status: 'error', storage: 'postgresql', error: 'Base de datos no disponible.' });
    }
});
if (config.useMemoryDatabase) {
    console.warn('[database] DATABASE_URL no configurada; usando almacenamiento temporal en memoria.');
