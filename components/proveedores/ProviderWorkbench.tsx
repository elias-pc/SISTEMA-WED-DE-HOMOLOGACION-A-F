...
function ProviderWorkbench({ provider, companyId, onChanged, onClose }: Props) {
  ...
  const uploadDocument = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setBusy(true); setMessage('');
    try {
      const metadata={ category: 'Expediente', originalName: file.name, mimeType: file.type || 'application/octet-stream' };
      const prepared=await api.prepareProviderDocument(provider.id,{...metadata,byteSize:file.size});
      if(prepared.mode==='server') await api.providerDocument(provider.id,{...metadata,contentBase64:await toBase64(file)});
      else {
        const form=new FormData();
        Object.entries(prepared.upload.fields).forEach(([key,value])=>form.append(key,value));
        form.append('file',file);
        const upload=await fetch(prepared.upload.url,{method:'POST',body:form});
        if(!upload.ok)throw new Error('No se pudo cargar el archivo en el almacenamiento seguro.');
        await api.completeProviderDocument(provider.id,{...metadata,documentId:prepared.documentId,storageKey:prepared.storageKey});
      }
      await load(); setMessage('Documento guardado de forma segura en el expediente.');
    }
