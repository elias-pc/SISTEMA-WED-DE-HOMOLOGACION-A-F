import type { AuthUser, DashboardResumen, Empresa, EstadoSeguimiento, ExpedienteProveedor, HistorialFlujo, ImportPreview, ProcesoHomologacion, Proveedor, ReporteEstadoProveedores, ReporteOperativo, ReporteProductividad, ResumenCartera, ResumenMiCartera, ResultadoAsignacion, TransicionDisponible } from '../types';
...
export const api={
 ...
 createProvider:(provider:Proveedor)=>request<{provider:Proveedor}>('/providers',{method:'POST',body:JSON.stringify(provider)}),
  ...
  prepareProviderDocument:(id:string,payload:{category:string;originalName:string;mimeType:string;byteSize:number;expiresOn?:string})=>request<{mode:'server'}|{mode:'neon-s3';documentId:string;storageKey:string;storageDriver:'neon-s3';upload:{url:string;fields:Record<string,string>}}>(`/providers/${encodeURIComponent(id)}/documents/upload-url`,{method:'POST',body:JSON.stringify(payload)}),
  completeProviderDocument:(id:string,payload:{category:string;originalName:string;mimeType:string;documentId:string;storageKey:string;expiresOn?:string})=>request<{document:ExpedienteProveedor['documents'][number]}>(`/providers/${encodeURIComponent(id)}/documents/complete`,{method:'POST',body:JSON.stringify(payload)}),
  ...
  myPortfolio:(processId:string)=>request<ResumenMiCartera>(`/providers/my-portfolio?processId=${encodeURIComponent(processId)}`),
  ...
  unassignProviders:(payload:{processId:string;providerIds:string[];executiveId:string;reason:string})=>request<ResultadoAsignacion>('/assignments/unassign',{method:'POST',body:JSON.stringify(payload)}),
