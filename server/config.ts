...
export const config = {
  ...
  documentStorageDriver: isTest ? 'local' : (process.env.DOCUMENT_STORAGE_DRIVER || 'local'),
  documentStoragePath: process.env.DOCUMENT_STORAGE_PATH || './data/uploads',
  documentStorageBucket: process.env.NEON_STORAGE_BUCKET || '',
...
export function validateNotificationConfiguration() {
  if (!config.whatsappEnabled) return { enabled: false, missing: [] as string[] };
