...
import { availableTransitions, type WorkflowState } from './workflow.js';
...
export function canAccessCompany(role:UserRole,companyIds:string[],companyId:string){return role==='supervisor_general'||role==='administradora'||companyIds.includes(companyId)}
export function canApplyTransition(role: UserRole, state: WorkflowState, transitionCode: string) {
 return availableTransitions(state, role).some((transition) => transition.code === transitionCode);
}
