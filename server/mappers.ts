...
export const mapProcess = (row: Record<string, unknown>) => ({ id: row.id, empresaId: row.company_id, codigo: row.code, nombre: row.name, fechaInicio: String(row.start_date).slice(0,10), fechaLimite: String(row.deadline).slice(0,10), estado: row.status, ejecutivaId: row.executive_id || undefined });
