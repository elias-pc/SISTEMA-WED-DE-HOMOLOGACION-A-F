...
function validateContent(content: Buffer) {
  validateByteSize(content.length);
}

function validateByteSize(byteSize: number) {
  if (!Number.isInteger(byteSize) || byteSize < 1 || byteSize > MAX_DOCUMENT_BYTES) throw new Error('El documento debe pesar entre 1 byte y 5 MB.');
}

function createStorageKey(providerId: string, documentId: string, originalName: string) {
  return `providers/${providerId}/${documentId}-${safeFileName(originalName)}`;
}

function neonBucket() {
  if (!config.documentStorageBucket) throw new Error('Falta configurar NEON_STORAGE_BUCKET para los entregables.');
...
async function writeLocalDocument(providerId: string, documentId: string, originalName: string, content: Buffer): Promise<StoredDocument> {
...
async function readLocalDocument(storageKey: string) {
...
export async function storeDocument(providerId: string, originalName: string, mimeType: string, contentBase64: string): Promise<StoredDocument> {
  ...
  if (config.isProduction) throw new Error('La producción requiere Neon Object Storage para guardar entregables.');
  return writeLocalDocument(providerId, documentId, originalName, content);
}

export async function createDocumentUpload(providerId: string, originalName: string, mimeType: string, byteSize: number) {
  if (config.documentStorageDriver !== NEON_DRIVER) return null;
  ...
  return { documentId, storageKey, storageDriver: 'neon-s3' as const, upload };
}

export async function confirmDocumentUpload(storageKey: string) {
...
export async function readDocument(storageDriver: string, storageKey: string) {
  if (storageDriver === 'local') return readLocalDocument(storageKey);
  if (storageDriver !== NEON_DRIVER) throw new Error('Controlador documental no compatible.');
  const result = await neonS3().send(new GetObjectCommand({ Bucket: neonBucket(), Key: storageKey }));
  if (!result.Body) throw new Error('El archivo no está disponible.');
