...
function limitLogin(request:import('express').Request,response:import('express').Response,next:import('express').NextFunction){
 const key=request.ip||'unknown',now=Date.now(),current=loginAttempts.get(key);
 if(!current||current.resetAt<=now){loginAttempts.set(key,{count:1,resetAt:now+15*60_000});return next()}
 if(current.count>=10)return response.status(429).json({error:'Demasiados intentos. Intenta nuevamente en 15 minutos.'});
...
authRouter.post('/login', limitLogin, validateBody(loginSchema), async (request, response) => {
 ...
 if (!row || !(await bcrypt.compare(password, row.password_hash))) return response.status(401).json({ error: 'Correo o contraseña incorrectos.' });
...
authRouter.get('/session', authenticateRequest, (request, response) => response.json({ user: (request as AuthenticatedRequest).user }));
