...
async function applyPlan(user: SessionUser, processId: string, plan: PlannedAssignment[], reason: string) {
  ...
  try {
    ...
    for (const assignment of activeAssignments.rows) {
      ...
      const ids = activeByProvider.get(providerId) || new Set<string>();
    ...
    for (const item of plan) {
      ...
      const active = activeByProvider.get(item.providerId) || new Set<string>();
      if (active.has(item.executiveId)) { unchanged += 1; continue; }
      assigned += 1;
