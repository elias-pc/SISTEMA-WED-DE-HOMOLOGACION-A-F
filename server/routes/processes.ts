...
const schema=z.object({ id:z.string().min(2).max(80).optional(),empresaId:z.string().min(1),codigo:z.string().min(3).max(80),nombre:z.string().min(3).max(180),fechaInicio:z.string().date(),fechaLimite:z.string().date(),estado:z.enum(['Planificación','En curso','Suspendido','Finalizado']).default('Planificación'),ejecutivaId:z.string().min(1).optional() }).refine(v=>v.fechaLimite>=v.fechaInicio,{message:'La fecha límite debe ser posterior al inicio.',path:['fechaLimite']});
processesRouter.post('/',requireRoles('supervisor_general','administradora'),validateBody(schema),async(request,response)=>{
 const user=(request as AuthenticatedRequest).user,b=request.body;if(!canAccessCompany(user,b.empresaId))return response.status(403).json({error:'Empresa fuera de alcance.'});
 const result=await pool.query('INSERT INTO homologation_processes(id,company_id,code,name,start_date,deadline,status,executive_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',[b.id||randomUUID(),b.empresaId,b.codigo,b.nombre,b.fechaInicio,b.fechaLimite,b.estado,b.ejecutivaId||null]);
