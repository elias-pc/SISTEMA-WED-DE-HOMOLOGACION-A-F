...
providersRouter.get('/',async(request,response)=>{
 ...
 if(!canAccessCompany(user,scope.company_id)&&!(user.role==='ejecutiva'&&scope.executive_id===user.id))return response.status(403).json({error:'Proceso fuera de tu alcance.'});
...
function canAccessProvider(user:SessionUser,row:Record<string,unknown>){
 ...
 if(user.role==='ejecutiva')return (row.assigned_executive_id||row.executive_id)===user.id;
