...
const executiveStatuses=['Contactado','No encontrado','Formulario enviado','Formulario respondido'] as const;
const supervisorStatuses=['En coordinación','No se ubica','Visita no realizada','Desestimado','Visita realizada'] as const;
const generalStatus=(status:string)=>status==='Visita realizada'?'Homologado':status==='Desestimado'?'Vencido':['No encontrado','No se ubica','Visita no realizada'].includes(status)?'Observado':'En proceso';
...
providersRouter.patch('/:id/status',requireRoles('ejecutiva','supervisor_empresa','supervisor_general'),validateBody(statusSchema),async(request,response)=>{
 const user=(request as AuthenticatedRequest).user,status=request.body.estado;
 ...
 if(!canAccessCompany(user,row.company_id)&&!(isExecutive&&row.executive_id===user.id))return response.status(403).json({error:'Proveedor fuera de tu alcance.'});
 if(!canUpdateStatus(user.role,status))return response.status(403).json({error:isExecutive?'La ejecutiva solo puede actualizar estados de contacto y formulario.':'Los supervisores solo pueden actualizar estados de coordinación y visita.'});
