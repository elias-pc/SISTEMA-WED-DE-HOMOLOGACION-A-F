...
async function seed() {
 ...
 try {
  ...
  for (const c of companies) await client.query('INSERT INTO companies(id,legal_name,tax_id,trade_name,contact_name,email,phone) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(id) DO NOTHING', c);
  for (const [id,name,email,password,role,companyId] of users) {
   ...
   if (companyId) await client.query('INSERT INTO user_companies(user_id,company_id) VALUES($1,$2) ON CONFLICT DO NOTHING', [id,companyId]);
  }
  ...
  await client.query('COMMIT'); console.log('Datos iniciales creados.');
