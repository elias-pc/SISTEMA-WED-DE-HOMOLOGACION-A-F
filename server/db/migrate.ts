...
async function migrate() {
  ...
  for (const filename of (await readdir(directory)).filter((file) => file.endsWith('.sql')).sort()) {
    ...
    try { await client.query('BEGIN'); await client.query(await readFile(path.join(directory, filename), 'utf8')); await client.query('INSERT INTO schema_migrations(filename) VALUES($1)', [filename]); await client.query('COMMIT'); console.log(`Migración aplicada: ${filename}`); }
